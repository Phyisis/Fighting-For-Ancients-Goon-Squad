...
LinkLuaModifier("modifier_spirits2", "heroes/drackos/modifiers/modifier_spirits2.lua", LUA_MODIFIER_MOTION_NONE)
	...
	Timers:CreateTimer(0, function()
		...
		if num < sDuration then
			...
			for _,v in pairs(heroes_in_radius) do
				if not v:HasModifier("modifier_spirits2") then
					v:AddNewModifier(hCaster, self, "modifier_spirits2", {["duration"] = duration})
				end
			end
			------------------------------------------------------------------------------------------------------------------------------
			return 0.03
...
function spirits2:OnUpgrade()
	hCaster = self:GetCaster()
	local spirits_ability = hCaster:FindAbilityByName("spirits")
	local spirits3_ability = hCaster:FindAbilityByName("spirits3")
	local spirits2_level = self:GetLevel()
	
	if spirits_ability ~= nil and spirits_ability:GetLevel() ~= spirits2_level then
		spirits_ability:SetLevel(spirits2_level)
	end

	if spirits3_ability ~= nil and spirits3_ability:GetLevel() ~= spirits2_level then
		spirits3_ability:SetLevel(spirits2_level)
	end
end